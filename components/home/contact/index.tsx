import React from 'react';
import { MdArrowOutward } from 'react-icons/md';
import { FiMapPin, FiPhone, FiMail } from "react-icons/fi";

interface ContactItem {
  id: number;
  icon: React.ReactNode;
  title: string;
  description: string;
}

const Contact: React.FC = () => {
  const contactItems: ContactItem[] = [
    {
      id: 1,
      icon: <FiMapPin size={28} className="text-white" />,
      title: "Office Location:",
      description: "I-102, Second Floor, Block I, Kirti Nagar, Delhi, 110015"
    },
    {
      id: 2,
      icon: <FiPhone size={28} className="text-white" />,
      title: "Call:",
      description: "+91 9220708032"
    },
    {
      id: 3,
      icon: <FiMail size={28} className="text-white" />,
      title: "Email:",
      description: "info@kiaiki.com"
    }
  ];
  return (
    <section className="bg-black">
      <div className='container mx-auto lg:px-16 pt-12 md:pt-24'>
        <div className="grid lg:grid-cols-2 gap-10">
          <div className="bg-white p-4 md:p-8 text-black lg:order-1 order-2" data-aos="fade-up">
            <form className="space-y-6">
              <input
                type="text"
                placeholder="Your Name"
                className="w-full border border-gray-300 p-3 rounded focus:outline-none"
              />
              <input
                type="email"
                placeholder="Your Email"
                className="w-full border border-gray-300 p-3 rounded focus:outline-none"
              />
              <div className="flex flex-col md:flex-row gap-4">
                <input
                  type="text"
                  placeholder="Your Phone"
                  className="w-full border border-gray-300 p-3 rounded focus:outline-none"
                />
                <input
                  type="text"
                  placeholder="Your Subject"
                  className="w-full border border-gray-300 p-3 rounded focus:outline-none"
                />
              </div>
              <textarea
                placeholder="Message"
                className="w-full border border-gray-300 p-3 h-32 rounded resize-none focus:outline-none"
              ></textarea>
              <button
                type="submit"
                className="bg-primary-accent hover:bg-black hover:text-white  px-5 py-3 text-white transition-all flex items-center"
              >
                SEND MESSAGE  &nbsp; <MdArrowOutward size={20} />
              </button>
            </form>
          </div>

          <div className="text-white flex flex-col justify-center space-y-6 mx-4 md:p-10 lg:order-2 order-1" data-aos="fade-up" data-aos-delay="300">
            <p className="text-sm tracking-widest text-primary-givry uppercase">Contact Us</p>
            <h2 className="text-3xl font-bold">
              We <span className="pl-2 text-white bg-transparent bg-[linear-gradient(270deg,transparent_54%,#EBD1AE_53%)] ms-2"> Create </span>  Your Space Better
            </h2>
            <p className="text-gray-300">
              Reach out to us for customized interior solutions that combine function, comfort, and elegance. Let’s bring your vision to life, one detail at a time.
            </p>

            <div className="md:gap-8 gap-2 text-sm grid md:grid-cols-1 max-w-sm">
              {contactItems.map((item) => (
                <div key={item.id} className="flex items-start gap-3">
                  <span className="bg-primary-accent p-5">{item.icon}</span>
                  <div>
                    <p><strong>{item.title}</strong></p>
                    <p>{item.description}</p>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Contact;
